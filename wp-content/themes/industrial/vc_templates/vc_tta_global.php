...
if($is_anps_accordion) {
	...
	/* ID */
	$accordion_id = bin2hex(random_bytes(3));
	...
	if( $atts['collapsible_all'] != 'true' ) {
		$parent = 'data-parent="#accordion-' . esc_attr($accordion_id) . '"';
	}

	$output .= '<div class="' . esc_attr($class) . '" id="accordion-' . esc_attr($accordion_id) . '" role="tablist" aria-multiselectable="true">';
	...
	/* ID */
	$tabs_id = bin2hex(random_bytes(3));
           ...
           $menu .= '<ul class="nav nav-tabs" role="tablist">';
          		...
          		foreach ($sections as $section) {
					...
					if( isset($section['tab_id']) && $section['tab_id'] != '' ) {
						...
						$section_id = $tabs_id . '-' . $section_count;
					}
